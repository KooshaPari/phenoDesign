...
case "$STACK" in
  ...
  node)
    # Detect bun vs npm
    PKG_MGR="npm"
    if [[ -f "bun.lock" ]] && command -v bun &>/dev/null; then PKG_MGR="bun"; fi

    if [[ "$PKG_MGR" == "bun" ]]; then
      run_check "install" "bun install" 1
      run_check "build" "bun run build" 2
      run_check "test-unit" "bun run test" 3
      run_check "lint" "bun run lint" 2
      run_check "fmt" "npx prettier --check '**/*.ts' 2>/dev/null || echo 'fmt skipped (no prettier)'" 2
      run_check "typecheck" "npx tsc --noEmit" 2
      run_check "test-e2e" "bun run test:e2e" 2 true
      run_check "test-perf" "bun run test:perf" 1 true
      run_check "test-mutation" "npx stryker run" 1 true
      run_check "coverage" "bun run test:coverage" 2 true
      run_check "audit" "bun audit" 1
    else
